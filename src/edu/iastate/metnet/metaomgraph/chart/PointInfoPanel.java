...
public class PointInfoPanel extends JPanel {
    ...
    public PointInfoPanel(MetaOmChartPanel aChartPanel) {
        ...
        jbrowseButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (selectedLocus.indexOf(";") >= 0) {
                    ...
                    target = (String) JOptionPane.showInputDialog(null, "Show Araport-JBrowse results for which ID?", "Araport-JBrowse", 3, null, ids, ids[0]);
                } else {
        ...
        thalemineButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (selectedLocus.indexOf(";") >= 0) {
                    String[] ids = selectedLocus.split(";");
                    target = (String) JOptionPane.showInputDialog(null, "Show Araport-ThaleMine results for which ID?", "Araport-ThaleMine", 3, null, ids, ids[0]);
                } else {
                    target = selectedLocus;
                }
        ...
        tairButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (selectedLocus.indexOf(";") >= 0) {
                    String[] ids = selectedLocus.split(";");
                    target = (String) JOptionPane.showInputDialog(null, "Show TAIR results for which ID?", "TAIR", 3, null, ids, ids[0]);
                } else {
                    target = selectedLocus;
                }
                String urlString = "http://www.arabidopsis.org/servlets/Search?type=general&name=" + target + "&action=detail&method=4&sub_type=gene";
                try {
                    BrowserLauncher launcher = new BrowserLauncher(null);
                    BrowserLauncherErrorHandler errorHandler = new BrowserLauncherDefaultErrorHandler();
                    BrowserLauncherRunner runner = new BrowserLauncherRunner(launcher, urlString, errorHandler);
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    ...
                    try {
                        ...
                        JOptionPane.showMessageDialog(myChartPanel, "Unable to launch web browser",
                                "Error", 0);
                        ex2.printStackTrace();
                    }
                }
        ...
        infoButton = new MenuButton("External web applications", MetaOmGraph.getIconTheme().getExternalSource(), infoPopupMenu);
    ...
    public String cleanHTML(String source) {
        String result;
        for (result = new String(source); result.indexOf("<script") >= 0; result = result.substring(0, result.indexOf("<script")) + result.substring(result.indexOf("</script>") + 9)) ;
        for (; result.indexOf("<SCRIPT") >= 0; result = result.substring(0, result.indexOf("<SCRIPT")) + result.substring(result.indexOf("</SCRIPT>") + 9));

        int hitPoint = 0;
        for (hitPoint = 0; result.indexOf("href=\"/", hitPoint) >= 0; result = result.substring(0, hitPoint) + "http://www.arabidopsis.org" + result.substring(hitPoint)) hitPoint = result.indexOf("href=\"", hitPoint) + "href=\"".length();
        for (hitPoint = 0; result.indexOf("href='/", hitPoint) >= 0; result = result.substring(0, hitPoint) + "http://www.arabidopsis.org" + result.substring(hitPoint)) hitPoint = result.indexOf("href='", hitPoint) + "href='".length();
        for (hitPoint = 0; result.indexOf("src=\"/", hitPoint) >= 0; result = result.substring(0, hitPoint) + "http://www.arabidopsis.org" + result.substring(hitPoint)) hitPoint = result.indexOf("src=\"", hitPoint) + "src=\"".length();
        for (hitPoint = 0; result.indexOf("src='/", hitPoint) >= 0; result = result.substring(0, hitPoint) + "http://www.arabidopsis.org" + result.substring(hitPoint)) hitPoint = result.indexOf("src='", hitPoint) + "src='".length();

        hitPoint = 0;
        for (; result.indexOf("<input type=\"button\"") >= 0; result = result.substring(0, hitPoint) + result.substring(result.indexOf(">", hitPoint) + 1)) hitPoint = result.indexOf("<input type=\"button\"");
        for (hitPoint = 0; result.indexOf("width=\"100%\"", hitPoint) >= 0; result = result.substring(0, hitPoint) + "width=\"602\"" + result.substring(hitPoint + "width=\"100%\"".length())) hitPoint = result.indexOf("width=\"100%\"", hitPoint);
    ...
    public void refresh() {
        if (myChartPanel.getSelectedPoint() == null) {
            ...
            //String v=myChartPanel.getFormatter().format(myChartPanel.getSelectedPoint().getX());
        ...
        if (myChartPanel.getSelectedSeries() < 0) {
            ...
            if (myChartPanel.getProject().getInfoColumnCount() <= 0) {
                ...
                for (int x = 0;
                     x < myChartPanel.getProject().getInfoColumnCount(); x++) {
                    String thisItem = myChartPanel.getProject().getRowName(myChartPanel.getSelectedRows()[myChartPanel.getSelectedSeries()])[x] + "";
                        ...
                        if (thisItem.indexOf(";") >= 0) {
                            ...
                            for (int i = 0; i < items.length; i++) {
                                if (Utils.getIDType(items[i]) == 1)
                                    if (selectedLocus == null) selectedLocus = items[i];
                                    else  selectedLocus = (selectedLocus + ";" + items[i]);
                            }
                        } else if (Utils.getIDType(thisItem) == 1) {
                            if (selectedLocus == null) selectedLocus = thisItem;
                            else selectedLocus = (selectedLocus + ";" + thisItem);
                        }
        ...
        if (tairItem.isEnabled())  tairItem.setToolTipText("Connect to the TAIR website for the selected gene");
        else tairItem.setToolTipText("No Locus ID found, can't connect to TAIR");

        if (jbrowseItem.isEnabled())  jbrowseItem.setToolTipText("Connect to the Araport-JBrowse website for the selected gene");
        else jbrowseItem.setToolTipText("No Locus ID found, can't connect to Araport-JBrowse");

        if (thalemineItem.isEnabled())  thalemineItem.setToolTipText("Connect to the Araport-ThaleMine website for the selected gene");
        else thalemineItem.setToolTipText("No Locus ID found, can't connect to Araport-ThaleMine");
    }

    public void reselect() {
        if (myChartPanel.getSelectedPoint() == null) return;
