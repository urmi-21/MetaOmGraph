...
public class TableSorter extends AbstractTableModel {
    ...
    protected Comparator getComparator(int column) {
        ...
        else return Comparable.class.isAssignableFrom(columnType) ? COMPARABLE_COMAPRATOR : LEXICAL_COMPARATOR;
    }
    ...
    private int[] getModelToView() {
        if (this.modelToView == null) {
            ...
            for (int i = 0; i < n; i++) this.modelToView[modelIndex(i)] = i;
        }
    ...
    public synchronized Object getValueAt(int row, int column) {
        return row <= this.tableModel.getRowCount() && column <= this.tableModel.getColumnCount() ? this.tableModel.getValueAt(this.modelIndex(row), column) : null;
    }
    ...
    private class MouseHandler extends MouseAdapter {
        public void mouseClicked(MouseEvent e) {
            ...
            if (column != -1) {
                ...
                PropertyChangeEvent evt = new PropertyChangeEvent(columnModel.getColumn(viewColumn), "sort", new Integer(oldstatus), new Integer(status));
