...
public class Utils {
    ...
    public static int getIDType(String geneID) {
        ...
        if (checkMe.matches(locusPattern))
            return 1;
        if ((checkMe.matches(affy8kPattern1)) ||
                (checkMe.matches(affy8kPattern2)))
            return 2;
        if ((checkMe.matches(affy25kPattern1)) ||
                (checkMe.matches(affy25kPattern2)))
            return 3;
        return -1;
    }


    public static String makeQueryString(ResultSet rs, String column, String name)
            throws SQLException {
        if (!rs.first())
            return "";
    ...
    public static File chooseFileToOpen(FileFilter filter, Component parent) {
        ...
        for (returnVal = 0; ((selectedFile == null) || (!selectedFile.exists())) &&
                (returnVal != 1); selectedFile = chooseDialog
                .getSelectedFile()) {
    ...
    public static File chooseFileToSave(FileFilter filter, String extension, Component parent, boolean forceExtension) {
        ...
        for (boolean ready = false; !ready; ) {
            ...
            if ((extension != null) && (!extension.equals(""))) {
                if (forceExtension) {
                    if (!getExtension(filename.toLowerCase()).equals(
            ...
            if (destination.exists()) {
                int overwrite = JOptionPane.showConfirmDialog(parent, filename +
                                " already exists.  Overwrite?", "Overwrite File", 1,
                        2);
    ...
    private static File macFileSave(final FileFilter filter, String extension, Component parent, boolean forceExtension) {
        String filename = null;
        File destination = null;
        ...
        if (parentFrame != null) {
            ...
            Dialog parentDialog = getDialogParent(parent);
            if (parentDialog != null) {
                chooseDialog = new FileDialog(parentDialog, "Save",
                        1);
            } else {
                chooseDialog = new FileDialog(new JFrame(), "Save",
                        1);
            }
        }
        if (getLastDir() != null) {
    ...
    public static void setLastDir(File newLastDir) {
        if ((newLastDir != null) && (newLastDir.exists()) &&
                (newLastDir.isDirectory()))
            lastDir = newLastDir;
    }
        ...
        for (int col = 0; col < table.getColumnCount(); col++) {
            ...
            int longest = myCol
                    ...
                    .getTableCellRendererComponent(table,
                            myCol.getHeaderValue(), false, false, 0, col)
    ...
    public static FileFilter createFileFilter(final String[] extensions,
        ...
        if ((extensions == null) || (extensions.length <= 0)) {
            throw new InvalidParameterException(
                    "extensions must not be null, and must have length>0");
        }
        ...
        FileFilter filter = new FileFilter() {
            ...
            public boolean accept(File f) {
                ...
                for (int x = 0; x < extensions.length; x++) {
                    if (getExtension(f).toLowerCase().equals(
                            extensions[x].toLowerCase()))
                        return true;
                }
                return false;
            }
    ...
    public static boolean onSamePath(TreeNode[] path1, TreeNode[] path2) {
        if ((path1 == null) || (path2 == null))
            return false;
        boolean result = true;
        int shortest = path1.length < path2.length ? path1.length - 1 :
                path2.length - 1;
    ...
    public static File unzip(File source, ZipEntry entry, String filenamePrefix) throws IOException {
        ...
        BufferedInputStream in = new BufferedInputStream(
                zipSource.getInputStream(entry));
    ...
    public static String compressString(String compressMe, int length) {
        ...
        StringBuilder result = new StringBuilder(compressMe.substring(0,
                length / 2));
        result.append("...");
    ...
    public static double complexStringMatch(Character[] s1, String s2, boolean ignoreCase) {
        ...
        for (int i = 0; i < s1.length; i++) {
            if (s1[i] != null) {
                if (ignoreCase) {
                    if (Character.toLowerCase(s1[i].charValue()) ==
    ...
    public static String convertNanoTimeToWords(long time, boolean verbose) {
        ...
        if (myTime < 7L) {
            ...
            return
                    myTime + " days " + hour + " hours " + min + " minutes " + sec + " seconds";
        }
        long day = myTime % 7L;
        myTime /= 7L;
        return myTime + " weeks " + day + " days " + hour + " hours " + min +
                " minutes " + sec + " seconds";
    }
    ...
    public static class Link {
        ...
        public Link(String html) {
            Pattern hrefRegex = Pattern.compile("(?<=href=\")[^\"]*(?=\")",
                    160);
    ...
    public static long stopWatch() {
        if (startTime == null) {
            throw new IllegalStateException(
                    "startWatch() must be called before stopWatch()");
        }
    ...
    public static File mandatoryFileOpen(String extension) {
        ...
        if (extension != null) {
            result = chooseFileToOpen(
                    createFileFilter(extension, extension + " files"),
                    null);
        } else {
            result = chooseFileToOpen();
        }
    ...
    public static String makeXMLElement(String name, String text, Map<String, String> attribs, int depth, boolean closed) {
        StringBuilder result = new StringBuilder();
        ...
        if (attribs != null) {
            ...
            for (String key : keys) {
                ...
                result.append(" " + key + "=\"" + attribs.get(key).replace("&", "&amp;").replace("\"", "&quot;") + "\"");
            }
        }
