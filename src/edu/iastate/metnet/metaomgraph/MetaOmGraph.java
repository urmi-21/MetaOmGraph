...
import edu.iastate.metnet.metaomgraph.ui.*;
import edu.iastate.metnet.metaomgraph.utils.*;
...
public class MetaOmGraph implements ActionListener {
    ...
    // create themes
    private static MOGColorThemes themeDefault = new MOGColorThemes(
			"default",
			Color.lightGray, Color.darkGray,
			Color.yellow, Color.blue, Color.magenta,
			Color.gray, Color.gray
	); //gui default theme
    private static MOGColorThemes themeLight = new MOGColorThemes(
            "light",
			Color.white, new Color(216, 236, 213),
			Color.DARK_GRAY, Color.PINK, Color.green,
			Color.WHITE, Color.WHITE
	);
    private static MOGColorThemes themeDark = new MOGColorThemes(
			"dark",
			new Color(153, 153, 153), new Color(204, 204, 204),
			Color.BLACK, new Color(0, 153, 102), Color.RED,
            new Color(153, 153, 153), new Color(153, 153, 153),
            Color.BLACK
	);
    private static MOGColorThemes themeSky = new MOGColorThemes(
			"sky",
			new Color(241, 250, 238), new Color(168, 218, 220),
			new Color(69, 123, 157), new Color(155, 197, 61), Color.green,
			Color.WHITE, Color.WHITE
	);
    ...
    /**
     * Creates a set of variables for the start up process. Sets up and configures the main window
     * and desktop Creates the menu bar with corresponding elements. Each element is derived from
     * the root element 'JMenuBar'. Each menu bar elements is associated with Action command which
     * will be called when the particular element is pressed.
     *
     * @param useBuffer - a boolean variable which handles exception
     */
...
//			mainMenuBar.add(historyMenu);
    ...
    /**
     * Prompts the user to save unsaved changes (if necessary), then exits the program. Doesn't exit
     * if the user cancels the save prompt, though.
     */
    ...
    /**
     * Prompts the user to save the active project. Doesn't check to see if there IS an active
     * project or not, though. If the user hasn't yet saved the project and elects to do so here, a
    ...
    /**
     * Worker that constructs a new project. This is implemented as a separate thread so that the
     * progress monitor will work.
     */
    public static class NewProjectWorker extends SwingWorker {
        ...
        /**
         * An array of row names to use in place of the row names in the source file (if any exist)
         */
        private Object[][] rowNames;

        /**
         * An array of column names to use in place of the column names in the source file (if any
         * exist)
         */
        ...
        /**
         * The boolean variable which determines whether to ignore the consecutive delimiters or
         * not
         */
         ...
         * Constructor.
         *
         * @param source      the delimited text file that contains the data for this project
         * @param infoColumns the number of info columns in the source file
         * @param delimiter   the character used as a delimiter in the source file
         * @param rowNames    the names of the rows (can be null). Replaces any row names in the
         *                    source file if non-null.
         * @param colNames    the names of the columns (can be null). Replaces any column names in
         *                    the source file if non-null.
         * @param extInfoFile an XML file that contains extended info for the new project (can be
         *                    null)
         */
         ...
         * Constructor.
         *
         * @param source                      the delimited text file that contains the data for
         *                                    this project
         * @param infoColumns                 the number of info columns in the source file
         * @param delimiter                   the character used as a delimiter in the source file
         * @param rowNames                    the names of the rows (can be null). Replaces any row
         *                                    names in the source file if non-null.
         * @param colNames                    the names of the columns (can be null). Replaces any
         *                                    column names in the source file if non-null.
         * @param extInfoFile                 an XML file that contains extended info for the new
         *                                    project (can be null)
         * @param ignoreConsecutiveDelimiters The boolean variable which determines whether to \
         *                                    ignore the consecutive delimiters or not
         * @param blankValue                  A double variable which determines the blank space
         *                                    value.
         */
        ...
        /**
         * If the project was created successfully, this sets up the extra information (row names,
         * column names, extended info) and displays the project table. Otherwise, it removes all
         * traces of the attempted operation.
         */
        @Override
    ...
    /**
     * Prompts the user to select a destination file, then saves the active project to that file.
     */
    ...
    public static class OpenProjectWorker extends AnimatedSwingWorker {
         ...
         * Constructor
         *
         * @param source the MetaOmGraph project file to open
         */
    ...
    /**
     * Closes the active project. If there are unsaved changes to the active project, the user is
     * prompted to save them. If the user cancels the save prompt, this returns false and does
     * nothing. Otherwise, it closes the project, removes any open JInternalFrames, and disables the
    ...
    /**
     * Fetches the WindowListener used to make a frame/dialog modal, but still be able to access the
     * Help frame. Normally, when a dialog is modal, it completely blocks the current process, thus
     * disabling access to all frames/dialogs besides the modal dialog and its children. By applying
     * this listener to a frame or dialog, when that frame/dialog is visible, it will disable only
     * the main MetaOmGraph window, thus allowing the user to still access other frames.
     *
     * @return a WindowListener that, when applied to a frame or dialog, will disable the main
     * MetaOmGraph window when the frame/dialog is visible
     */
     ...
     *
     * @throws PropertyVetoException if the proposed change to a property represents an unacceptable
     *                               value
     */
     ...
     *
     * @param newTheme instance of <code>IconTheme</code> class
     */
...
//        new MetaOmApplicationListener();
    ...
    /**
     * This method is used to update the logging file name for each open-project action It will
     * ensure that each project's action will be written to its own project directory's log folder
     */
