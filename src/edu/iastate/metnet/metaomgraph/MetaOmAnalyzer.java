...
public class MetaOmAnalyzer {
	...
	public static void pearsonAll(final MetaOmProject project, String geneList) {
		...
		final int[] entries = project.getGeneListRowNumbers(geneList);
		String message = project.getRowName(entries[0])[project.getDefaultColumn()] + "";
		final BlockingProgressDialog progress = new BlockingProgressDialog(MetaOmGraph.getMainWindow(), "Analyzing...",
				message, 0L, entries.length, true);
			...
			public Object construct() {
				try {
					...
					do {
						...
						progress.setMessage(
								rowNames[row][project.getDefaultColumn()] + " (" + row + "/" + entries.length + ")");
						for (int i = 0; (i < entries.length) && (!progress.isCanceled()); i++) {
							progress.setProgress(i);

							double[] data = project.getIncludedData(entries[i]);

							out.write("\t" + calcy.pearsonCorrelation(data, project.mayContainBlankValues(),
									project.getBlankValue()));
						}
						out.write("\r\n");
						row++;
	...
	public static void pearsonAll2(final MetaOmProject project, String geneList) {
		File dest = Utils.chooseFileToSave(Utils.createFileFilter("txt", "Tab-delimited text files"), "txt",
				MetaOmGraph.getMainWindow(), true);
		if (dest == null) {
			return;
				...
				try {
					...
					do {
						...
						for (int i = row; (i < entries.length) && (!progress.isCanceled()); i++) {
							...
							out.write("\t" + format.format(calcy.pearsonCorrelation(data,
									project.mayContainBlankValues(), project.getBlankValue())));
						}
						out.write("\r\n");
						row++;
	...
	public static void pearsonAll3(final MetaOmProject project, String geneList) {
		final File dest = Utils.chooseFileToSave(Utils.createFileFilter("txt", "Tab-delimited text files"), "txt",
				MetaOmGraph.getMainWindow(), true);
		if (dest == null) {
			return;
		}
		...
		SwingWorker analyzeWorker = new SwingWorker() {
			...
			public Object construct() {
				try {
					...
					do {
						...
						out.write("row");
						progress.setMessage(row + "/" + entries.length);
			...
			public void finished() {
				...
				if (!progress.isCanceled()) {
					progress.setMessage("Transposing Matrix");

					transposeMatrix(dest, new File(dest.getParent(), "transposed.txt"), project, entries);
				}
				progress.dispose();
			}
		};
	...
	public static void pearsonAll4(final MetaOmProject project, String geneList, final int nameCol) {
		File dest = Utils.chooseFileToSave(Utils.createFileFilter("txt", "Tab-delimited text files"), "txt",
				MetaOmGraph.getMainWindow(), true);
		if (dest == null) {
			return;
	...
	public static void pearsonAll5(final MetaOmProject project, String geneList, final int nameCol) {
		File dest = Utils.chooseFileToSave(Utils.createFileFilter("txt", "Tab-delimited text files"), "txt",
				MetaOmGraph.getMainWindow(), true);
		if (dest == null) {
			return;
